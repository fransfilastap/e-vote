            ...
            <div class="p-6 bg-white border-b border-gray-200 sm:px-20">
                ...
                <div class="mt-6 text-gray-500">
                    <div class="inline-block min-w-full overflow-hidden rounded-lg shadow">
                                ...
                                @foreach ($options as $option)
                                    <tr>
                                        <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ __($option->label) }}
                                            </p>
                                        </td>
                                        <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ __($option->attribute_1) }}
                                            </p>
                                        </td>
                                        <td class="px-5 py-5 text-sm bg-gray-100 border-b border-l border-gray-200">
                                            <div class="flex flex-col space-y-2">
                                                <button wire:click="requestUpdate({{ $option }})"
                                                ...
                                                <button wire:click='requestDelete({{ $option }})'
                        ...
                        <livewire:vote.vote-options-create />
                        <livewire:vote.vote-options-update />
                    </div>
                </div>
            </div>
        </div>
